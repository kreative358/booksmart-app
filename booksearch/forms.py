...
class BookSearch(forms.Form):
    ...
    intitle = forms.CharField(required=False,
        max_length=50, label='Search by book title', widget=forms.TextInput(attrs={'id':'input_text_bs_intitle', 'placeholder': 'field to enter full title of the book'})) #'autofocus': True, 'required': False

    inauthor = forms.CharField(required=False,
        max_length=50, label='Search by book author', widget=forms.TextInput(attrs={'id':'input_text_bs_inauthor', 'placeholder': 'field to enter full name of the author',  'pattern':'^(\w.+\s).+|(["\'])(\w.+\s).+(["\'])$', 'title':'must contain at least two words\nIf it is full name please add "" e.g. "Mark Twain"'}))

    search_query = forms.CharField(required=False,
        max_length=50, label='Search by any parameters', widget=forms.TextInput(attrs={'id':'input_text_bs_search_query', 'placeholder': 'field to enter e.g. part author name or title', 'pattern':'[A-Za-z0-9._-]', 'title':'for better results you can try add "" for query e.g. "book example" \nand to search for all entries that contain the exact phrase \n e.g. "Elizabeth Bennet" and the word "Darcy" \nbut do not contain the word "Austen", \nuse :"Elizabeth+Bennet"+Darcy-Austen'})) #, 'autofocus': True
    
    volumeId = forms.CharField(required=False,
        max_length=12, label='Search by googlebooks id', widget=forms.TextInput(attrs={'id':'input_text_bs_volumeId','placeholder': 'field to enter googlebooks id of the book', 'autofocus': True, 'pattern':'^[A-Za-z0-9._-]{12}$ ', 'title':'12 characters'}))  
    ...
    search_startIndex = forms.IntegerField(required=False,
        label='Choose a start index search', min_value=0, widget=forms.NumberInput(attrs={'id':'input_integer_bs_search_startIndex'}))

    search_inpublisher = forms.CharField(required=False,
        max_length=50, label='Search by book publishers', widget=forms.TextInput(attrs={'id':'input_text_bs_search_inpublisher', 'placeholder': "field to enter name of the book's publisher"}))  # , 'autofocus': True
    
    bl_volumeId = forms.CharField(required=False,
        label='Paste the link of the book', min_length=12, widget=forms.TextInput(attrs={'id':'bs_input_text_bl_volumeId', 'placeholder': "field to enter link to the book from google books", 'pattern':'^http:\/\/?\/.*id=([A-Za-z0-9._-]{12}).*|.*\/([A-Za-z0-9._-]{12})\/.*|.*\?id=([A-Za-z0-9._-]{12}\&).*$'}))  #'autofocus': True,
